...
import { Clock, X, Plus, CalendarDays, Fingerprint, CalendarCheck, Filter, User } from "lucide-react";
...
export default function AttendancePage() {
  ...
  const filteredAttendanceRecords = useMemo(() => {
    return attendanceRecords.filter((record: Attendance) => {
      ...
      if (selectedEmployeeId !== "ALL" && record.employeeId?._id !== selectedEmployeeId) {
        return false;
      }
      return true;
    });
  }, [attendanceRecords, selectedMonth, selectedStatus, selectedEmployeeId]);
  ...
  const columns: Column<Attendance>[] = [
    ...
    },
    {
      header: "Total Hours",
      cell: (row) => {
        const mins = (row as any).totalWorkingMinutes;
        if (mins && mins > 0) {
          const hrs = Math.floor(mins / 60);
          const rem = mins % 60;
          return <span className="font-bold text-blue-600 dark:text-blue-400 font-mono text-xs">{hrs}h {rem}m</span>;
        }
        if (row.checkIn && !row.checkOut) {
          return <span className="text-[10px] font-bold text-emerald-500 uppercase tracking-wide">Active Shift</span>;
        }
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      {/* Month, Employee & Status Filter Controls */}
      <div className="flex flex-wrap items-center justify-between gap-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-900 dark:text-white">Attendance Organization & Filters</h3>
            <p className="text-[11px] text-slate-500">Filter logs date-wise by month, status, or specific employee</p>
          </div>
        </div>
      ...
      <AdminDataTable
        ...
        columns={columns}
        data={isLoading ? [] : filteredAttendanceRecords}
